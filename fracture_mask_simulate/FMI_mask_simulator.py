...
class FMI_mask_simulator:
    ...
    def generate_sin_crack_by_config(self, config_crack={}):
        ...
        """
        width, height, crack_x_shift, noise_level, crack_width, crack_value = config_crack['width_background'], config_crack['height_background'], config_crack['crack_x_shift'], config_crack['noise_level'], config_crack['crack_width'], config_crack['crack_value']

        # 1. 创建空白背景图像
        img = np.zeros((height, width), dtype=np.uint8)
        img.fill(self.VALUE_BACKGROUNG)
        ...
        print(f'{crack_x_shift:.4f}', x_shift)
    ...
    def add_random_crack_cut_off(self, img=np.array([]), x_shift=0.0, cut_off_num=1, cut_off_width=0.05, noise_level=0.2):
        ...
        # 11. 将截断区域恢复为背景值
        img[mask == 1] = self.VALUE_BACKGROUNG

        return img

    def generate_crack_by_config(self, crack_config={}, crack_ratio_setting=0.04, trying_num=20):
        # 根据裂缝配置尝试生成新的裂缝
        ################# ['width_background'], ['height_background'], ['crack_x_shift'], ['noise_level'], ['crack_width'], ['crack_break_ratio'], ['crack_value']
        crack_config_default = self.get_config_norm_crack()

        if crack_config:
            crack_config_default.update(crack_config)

        ########## 这个config，是用来做出来给深度学习模型的，是模型的训练数据
        crack_config_target = {
            'width_background': crack_config_default['width_background'],  # 背景图像的宽度
            'height_background': crack_config_default['height_background'],  # 背景图像的高度
            'crack_x_shift': crack_config_default['crack_x_shift'],  # 正弦裂缝沿着x方向的偏移角度，0-1，百分比配置，这个主要是
            'noise_level': 0,  # 裂缝之间的噪声水平
            'crack_width': 10,  # 裂缝的宽度
            'crack_value': crack_config_default['crack_value'],  # 裂缝填充信息，是高导缝、高阻缝、层理缝或者是诱导缝、断层
        }

        crack_ratio = 0
        crack_random = np.array([])
        while(crack_ratio<crack_ratio_setting and trying_num > 0):
            crack_random = self.generate_sin_crack_by_config(crack_config_default)
    ...
    def generate_bedding_by_config(self, bedding_config={}):
        """
        generate a new bedding pic by config
        """
        pass
    ...
    def get_config_crack_random(self):
        ...
        crack_norm = {
            ...
            'crack_x_shift': np.random.random(),           # 正弦裂缝沿着x方向的偏移角度，0-1，百分比配置，这个主要是
            'noise_level': 0.15+0.15*np.random.random(),            # 裂缝之间的噪声水平
            'crack_width': np.random.randint(22, 34),              # 裂缝的宽度
            'crack_value': self.VALUE_HIGH_CONDUCTIVITY,        # 裂缝填充信息，是高导缝、高阻缝、层理缝或者是诱导缝、断层
    ...
    def get_config_crack_wide(self):
        ...
        crack_config_wide = {
            ...
            'crack_width': np.random.randint(50, 100),              # 裂缝的宽度
            'crack_value': self.VALUE_HIGH_CONDUCTIVITY,        # 裂缝填充信息，是高导缝、高阻缝、层理缝或者是诱导缝、断层
            'crack_cut_off': np.random.choice([0, 1, 1, 1, 2, 2, 3, 3, 3, 4]),             # 裂缝可能不是连续的，裂缝水平方向进行截断的个数，一般是1-2次，
